import React from 'react';
import { NordicSwooshConfig, NordicSwooshSize } from '../types';

export const BRAND_COLORS = {
  lightTeal: '#8FB9B8',
  darkTeal: '#0B4F60',
  darkGray: '#4C4C4C',
  lightGray: '#A8A8A8',
  white: '#FFFFFF',
};

// Raw Path Strings from Official Adobe Illustrator Export
const PATH_DOLOMITI = "M626.15,169.48h-21.71v83.5h21.71v-83.5ZM579.48,187.86h17.94v-18.38h-57.58v18.38h17.94v65.11h21.7v-65.11ZM532.99,169.48h-21.7v83.5h21.7v-83.5ZM396.65,252.98h21.59l6.98-48.06h.22l19.16,48.06h8.64l20.04-48.06h.22l6.09,48.06h21.71l-12.62-83.5h-21.59l-17.94,44.63-16.94-44.63h-21.37l-14.17,83.5ZM346.36,187.75c12.96,0,23.59,10.52,23.59,22.04,0,14.4-10.63,24.92-23.59,24.92s-23.59-10.52-23.59-24.92c0-11.52,10.63-22.04,23.59-22.04M346.36,166.71c-24.03,0-46.18,17.72-46.18,42.97,0,27.02,19.38,46.07,46.18,46.07s46.18-19.05,46.18-46.07c0-25.25-22.15-42.97-46.18-42.97M272.52,169.48h-21.7v83.5h47.73v-18.38h-26.02v-65.11ZM194.16,187.75c12.96,0,23.59,10.52,23.59,22.04,0,14.4-10.63,24.92-23.59,24.92s-23.59-10.52-23.59-24.92c0-11.52,10.63-22.04,23.59-22.04M194.16,166.71c-24.03,0-46.18,17.72-46.18,42.97,0,27.02,19.38,46.07,46.18,46.07s46.18-19.05,46.18-46.07c0-25.25-22.15-42.97-46.18-42.97M90.3,187.86h5.09c13.95,0,23.81,8.42,23.81,23.37,0,16.28-11.07,23.37-24.03,23.37h-4.87v-46.73ZM68.6,252.98h30.78c23.14,0,42.41-18.27,42.41-41.75s-19.16-41.75-42.41-41.75h-30.78v83.5Z";

const PATH_NORDICSKI = "M705.2,269.93h-9.3v83.5h9.3v-83.5ZM637.51,305.7v-35.77h-9.3v83.5h9.3v-35.99l2.33-2.32,34.22,38.32h12.85l-40.53-44.63,39.2-38.87h-12.51l-35.55,35.77ZM612.34,281.45c-4.43-8.19-12.29-12.96-21.59-12.96-12.4,0-23.37,8.31-23.37,21.26,0,11.96,8.42,16.94,18.16,21.37l5.09,2.21c7.75,3.54,15.39,6.87,15.39,16.61s-8.2,16.28-17.16,16.28-15.5-6.98-16.72-15.5l-9.08,2.55c2.66,12.96,12.62,21.59,26.02,21.59,14.51,0,26.24-11.19,26.24-25.8,0-13.29-9.19-18.71-20.04-23.48l-5.54-2.44c-5.65-2.55-13.07-5.87-13.07-13.07,0-7.64,6.42-12.96,13.84-12.96s11.07,3.32,14.4,9.08l7.42-4.76ZM552.84,276.69c-7.42-5.54-15.61-8.19-24.92-8.19-23.92,0-43.52,19.6-43.52,43.52s19.93,42.86,43.52,42.86c8.86,0,17.72-2.99,24.92-8.19v-11.52c-6.64,6.64-15.84,11.07-25.25,11.07-18.71,0-33.89-16.17-33.89-34.55s15.06-34.55,33.66-34.55c9.63,0,18.83,4.21,25.47,11.07v-11.52ZM471.65,269.93h-9.3v83.5h9.3v-83.5ZM395.9,344.79v-66.22h6.76c9.86,0,18.27,1.33,25.91,8.08,7.2,6.31,10.85,15.5,10.85,25.03s-3.54,18.05-10.41,24.47c-7.64,7.2-16.17,8.64-26.36,8.64h-6.76ZM386.6,353.43h15.5c13.07,0,23.03-1.66,33-10.52,9.19-8.19,13.62-18.94,13.62-31.12s-4.32-23.59-13.84-31.78c-9.86-8.42-20.04-10.08-32.56-10.08h-15.72v83.5ZM337.12,316.77h2.32l25.25,36.65h11.41l-26.58-37.43c12.85-1.11,20.93-10.19,20.93-23.03,0-18.83-14.73-23.03-30.56-23.03h-12.07v83.5h9.3v-36.65ZM337.12,278.57h2.77c11.19,0,21.26,1.33,21.26,15.06,0,12.96-10.63,14.95-21.15,14.95h-2.88v-30.01ZM271.04,277.13c19.16,0,34.22,15.62,34.22,34.55s-15.28,34.55-34.22,34.55-34.22-15.73-34.22-34.55,15.06-34.55,34.22-34.55M271.04,268.49c-23.81,0-43.52,19.38-43.52,43.3s19.71,43.08,43.52,43.08,43.52-19.05,43.52-43.08-19.71-43.3-43.52-43.3M139.59,353.43h9.3v-64.34l65.11,68.1v-87.26h-9.3v64.67l-65.11-68.22v87.04Z";

const PATH_TRACK_UPPER = "M698.66,238.5s9.42,1.65,29.07,6.68c27.25,6.97,68.5,20.01,80.77,40.36,18.39,30.51-33.04,67.62-33.04,67.62h39.77s41.38-36.18,13.77-74.04c-24.17-33.14-130.34-40.63-130.34-40.63";

const PATH_TRACK_LOWER = "M631.35,231.28s117.48,21.38,140.43,50.78c22.95,29.4-57.21,71.37-57.21,71.37h39.77s64.63-34.58,37.02-72.43c-24.17-33.14-160.01-49.71-160.01-49.71";

const PATH_SKIER_CHEST = "M755.61,108.68s-.15.14-.6.71l-2.18-3.02c1.76-1.9,2.27-2.39,2.27-2.39v2.22c.08,1.26.35,2.08.51,2.48h0ZM761.72,112.73c.21-1.34-2.25-4.93-2.25-4.93v-.05c.14-1.1,1.76-15.54.95-17.06-.82-1.53-3.06.42-3.06.42l-10.06,8.39-15.11-16.62s1.51-1.65.42-2.6c-1.09-.95-4.73,2.13-3.78,3.36.95,1.23,2.29.19,2.29.19l13.99,17.52-4.35,4.16c-.31-.02.83-5.84.46-5.85-.93-2.05-1.47-4.56-4.7-6.31-2.87-1.43-9.46.86-9.46.86,0,0-2.84,6.74-2.62,7.3.17.43.41,2.74.51,3.73-3.03.82-5.91,1.86-8.19,3.21-4.1-3.01-15.05-10.81-17.85-10.46-4.89.62-9.46,3.04-12.12,5.21l-2.64,2.17s-.39-.41-1.55-.5l-1.09-33.91s2.6-.99,1.51-2.35c-1.09-1.36-4.98-.11-4.53,1.09.5,1.34,1.34,1.26,1.34,1.26l-.17,34.08c-1.48.2-2.74.77-2.98,2.12-.3,1.71,1.29,5.68,1.29,5.68.54.63,2.02,1.43,2.02,1.43v1.18s1,1.49,2.27.76c.65-.43.65-.92.65-.92l-.09-1.07c.24-.02.48-.05.78-.07,1.15-.11,2.7-4.42,2.86-4.89.12-.1,3-2.16,3-2.16.95-.59,1.93-1.11,2.81-1.37,2.86-.82,5.42-.61,5.42-.61,0,0,3.45,1.11,4.99,2.54.84.79,4.28,4.91,7.19,8.45-.19.31-.37.64-.55.96.23.49,6.88,10.59,13.66,12.5,6.29,1.77,11.84-1.13,12.75-1.64,4.15-5.74,8.62-11.59,10.8-14.43,1.41-1.65,2.97-3.44,4.59-5.2.11-.12,2.59,2.95,2.59,2.95-.34.51-.39.67-.25.76-.09.65.65,1.59,1.29,2.28,0,.25.02,1.34.26,1.7.27.41,3.54,2.04,3.54,2.04,0,0,.52-.42,1.13-1.04.29.07.66.06.95-.31.56-.73.21-1.09.21-1.09.33-.49.86-2.49.92-2.86";

const PATH_SKIER_BODY = "M766.69,235.77c-.54-1.36-1.77-.54-2.04.95-.27,1.5-1.23,3.41-4.77,2.86-3.38-.52-9.89-3.62-19.57-5.57.13-.01.22-.02.22-.02l.68-.95s-.95-.68-2.04-1.91c-1.09-1.23-1.91-5.04-2.45-8.99-.43-3.15.44-9.07.81-11.38.05-.03.14-.07.14-.07,0,0,.14-6.13,2.86-15.4,2.73-9.27,4.91-16.08,5.18-17.31.27-1.23-4.2-11.64-8.15-19-3.54-6.59-12.97-12.21-12.97-12.21,0,0,1.04-3.12,4.1-8.36l-.08.12s-8.02,1.98-14.89-3.18c-6.87-5.16-7.88-11.34-7.88-11.34,0,0,.05-.12.12-.31-3.02,6.71-5.03,12.98-5.01,12.98-2.92,4.11-11.76,16.62-12.89,18.66-1.36,2.45-5.18,10.63-5.18,10.63l-11.31,11.18s-6.54,14.31-11.45,18.26c-4.91,3.95-6.41,5.59-6.41,5.59,0,0-5.18,4.09-8.45,5.59-3.27,1.5-7.63,4.64-8.31,5.73-.68,1.09-1.23,2.18-1.23,2.18l.2.2c-9.05,5.7-19.96,13.11-20.64,12.88-.82-.27.14-4.63.14-4.63,0,0-1.5,0-2.05.41-.54.41-2.59,6-1.64,6.95.96.95,3.14,1.09,4.77.41,1.64-.68,14.58-8.59,20.31-11.72,1.4-.77,3.56-1.87,6.02-3.08,1.14-.41,2.44-.95,3.93-1.69,1.06-.53,1.98-1.07,2.83-1.61,5.83-2.81,11.21-5.34,11.21-5.34l4.5-1.91s-.14-1.77-.82-1.36c-.27.16-2.51.51-5.68,1.21.74-.7,1.44-1.37,1.91-1.82,2.97-2.35,7.49-6.55,10.31-8.52,4.09-2.86,11.86-7.91,11.86-7.91,0,0,1.23-5.86,1.91-6.54.68-.68,8.27-12.86,16.04-18.44,7.77-5.59,9.02-5.37,9.02-5.37,0,0,12.96,5.14,17.73,10.73,4.77,5.59,7.63,11.31,7.9,11.59.27.27-3.41,3-3.54,4.9-.14,1.91,2.18,9.95,1.64,15.95-.3,3.34-.82,6.02-1.21,7.76-.42,1.13-1.47,4.08-2.2,6.55-.95,3.27-1.36,4.23-1.91,5.04-.55.82-.95,4.77.95,8.18,1.42,2.54,2.54,4.46,3.86,5.56-10.36-1.22-23.22-1.57-23.22-1.34l-2.04.41s12.4,1.23,23.85,3.54c11.45,2.32,27.67,6.27,27.67,6.27,0,0,2.32,0,3.95-.41,1.64-.41,3.95-4.22,3.41-5.59";

// Helper to make SVG Data URIs for img src compatibility
function createSvgDataUrl(svgString: string): string {
  return `data:image/svg+xml;charset=utf-8,${encodeURIComponent(svgString)}`;
}

const SVG_FULL_COLOR = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="55 45 835 315"><path fill="${BRAND_COLORS.lightTeal}" d="${PATH_DOLOMITI}"/><path fill="${BRAND_COLORS.darkTeal}" d="${PATH_NORDICSKI}"/><path fill="${BRAND_COLORS.lightTeal}" d="${PATH_TRACK_UPPER}"/><path fill="${BRAND_COLORS.lightTeal}" d="${PATH_TRACK_LOWER}"/><path fill="${BRAND_COLORS.lightTeal}" d="${PATH_SKIER_CHEST}"/><path fill="${BRAND_COLORS.darkTeal}" d="${PATH_SKIER_BODY}"/></svg>`;

const SVG_FULL_NEGATIV = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="55 45 835 315"><path fill="${BRAND_COLORS.lightTeal}" d="${PATH_DOLOMITI}"/><path fill="${BRAND_COLORS.white}" d="${PATH_NORDICSKI}"/><path fill="${BRAND_COLORS.lightTeal}" d="${PATH_TRACK_UPPER}"/><path fill="${BRAND_COLORS.lightTeal}" d="${PATH_TRACK_LOWER}"/><path fill="${BRAND_COLORS.lightTeal}" d="${PATH_SKIER_CHEST}"/><path fill="${BRAND_COLORS.white}" d="${PATH_SKIER_BODY}"/></svg>`;

const SVG_FULL_GRAU = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="55 45 835 315"><path fill="${BRAND_COLORS.lightGray}" d="${PATH_DOLOMITI}"/><path fill="${BRAND_COLORS.darkGray}" d="${PATH_NORDICSKI}"/><path fill="${BRAND_COLORS.lightGray}" d="${PATH_TRACK_UPPER}"/><path fill="${BRAND_COLORS.lightGray}" d="${PATH_TRACK_LOWER}"/><path fill="${BRAND_COLORS.lightGray}" d="${PATH_SKIER_CHEST}"/><path fill="${BRAND_COLORS.darkGray}" d="${PATH_SKIER_BODY}"/></svg>`;

const SVG_KURVE = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="625 220 275 140"><path fill="${BRAND_COLORS.lightTeal}" d="${PATH_TRACK_UPPER}"/><path fill="${BRAND_COLORS.lightTeal}" d="${PATH_TRACK_LOWER}"/></svg>`;

const SVG_KURVE_LANGLAEUFER = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="625 45 275 315"><path fill="${BRAND_COLORS.lightTeal}" d="${PATH_TRACK_UPPER}"/><path fill="${BRAND_COLORS.lightTeal}" d="${PATH_TRACK_LOWER}"/><path fill="${BRAND_COLORS.lightTeal}" d="${PATH_SKIER_CHEST}"/><path fill="${BRAND_COLORS.darkTeal}" d="${PATH_SKIER_BODY}"/></svg>`;

/**
 * Official Dolomiti NordicSki Brand Assets
 * 100% Pure Vector SVG Data URLs - 0 Raster Artifacts, Crisp at any DPI
 */
export const OFFICIAL_ASSET_PATHS = {
  logoFarbe: createSvgDataUrl(SVG_FULL_COLOR),
  logoNegativ: createSvgDataUrl(SVG_FULL_NEGATIV),
  logoWhiteSvg: createSvgDataUrl(SVG_FULL_NEGATIV),
  logoGrau: createSvgDataUrl(SVG_FULL_GRAU),
  kurve: createSvgDataUrl(SVG_KURVE),
  kurveLanglaeufer: createSvgDataUrl(SVG_KURVE_LANGLAEUFER),
};

/**
 * 1. Nordic Swoosh / Kurve Vector Component
 */
export const NordicSwooshGraphic: React.FC<{
  className?: string;
  variant?: 'swoosh_only' | 'swoosh_skier';
  style?: React.CSSProperties;
  color?: string;
}> = ({
  className = "w-full h-auto",
  variant = 'swoosh_only',
  style,
  color
}) => {
  if (variant === 'swoosh_skier') {
    return <DolomitiSkierTrackEmblem className={className} style={style} color={color} />;
  }

  const trackColor = color || BRAND_COLORS.lightTeal;

  return (
    <svg 
      viewBox="625 220 275 140" 
      className={`object-contain pointer-events-none select-none ${className}`}
      style={style}
    >
      <path fill={trackColor} d={PATH_TRACK_UPPER} />
      <path fill={trackColor} d={PATH_TRACK_LOWER} />
    </svg>
  );
};

export const DolomitiCurvesVector = NordicSwooshGraphic;
export const DolomitiSwooshVector = NordicSwooshGraphic;

/**
 * 2. Official Skier & Track Emblem ("Langläufer mit Kurve")
 */
export const DolomitiSkierTrackEmblem: React.FC<{
  className?: string;
  style?: React.CSSProperties;
  color?: string;
  usePng?: boolean;
}> = ({
  className = "w-16 h-16",
  style,
  color
}) => {
  const trackColor = color || BRAND_COLORS.lightTeal;
  const chestColor = color || BRAND_COLORS.lightTeal;
  const bodyColor = color || BRAND_COLORS.darkTeal;

  return (
    <svg 
      viewBox="625 45 275 315" 
      className={`object-contain shrink-0 ${className}`}
      style={style}
    >
      <path fill={trackColor} d={PATH_TRACK_UPPER} />
      <path fill={trackColor} d={PATH_TRACK_LOWER} />
      <path fill={chestColor} d={PATH_SKIER_CHEST} />
      <path fill={bodyColor} d={PATH_SKIER_BODY} />
    </svg>
  );
};

export const DolomitiSkierVector = DolomitiSkierTrackEmblem;

/**
 * 3. Dynamic Nordic Swoosh Overlay Component
 */
export const NordicSwooshOverlay: React.FC<{
  config?: NordicSwooshConfig;
  className?: string;
}> = ({ config, className = "" }) => {
  if (!config || !config.enabled || config.position === 'none') return null;

  const opacityVal = (config.opacity ?? 85) / 100;

  const sizeMap: Record<NordicSwooshSize, string> = {
    sm: 'w-28 max-w-[140px]',
    md: 'w-48 max-w-[220px]',
    lg: 'w-72 max-w-[320px]',
    xl: 'w-96 max-w-[420px]',
    '2xl': 'w-[520px] max-w-[600px]',
    custom: '',
  };

  const sizeClass = config.customWidthPx ? '' : sizeMap[config.size || 'md'];

  let posClasses = 'absolute z-10 pointer-events-none select-none transition-all';
  let imgTransform = '';

  switch (config.position) {
    case 'top_left':
      posClasses += ' top-0 left-0';
      imgTransform = 'scaleX(-1)';
      break;
    case 'top_right':
      posClasses += ' top-0 right-0';
      imgTransform = 'none';
      break;
    case 'bottom_left':
      posClasses += ' bottom-0 left-0';
      imgTransform = 'scaleX(-1)';
      break;
    case 'bottom_right':
      posClasses += ' bottom-0 right-0';
      imgTransform = 'none';
      break;
    default:
      posClasses += ' top-0 right-0';
      imgTransform = 'none';
  }

  const customWidthStr = config.customWidthPx ? `${config.customWidthPx}px` : undefined;

  const customStyle: React.CSSProperties = {
    opacity: opacityVal,
    width: customWidthStr,
    maxWidth: '90%',
  };

  const imgStyle: React.CSSProperties = {
    ...(customWidthStr ? { width: customWidthStr, maxWidth: '100%' } : {}),
    ...(imgTransform !== 'none' ? { transform: imgTransform } : {}),
  };

  return (
    <div className={`${posClasses} ${className}`} style={customStyle}>
      <NordicSwooshGraphic 
        variant={config.variant === 'swoosh_skier' ? 'swoosh_skier' : 'swoosh_only'}
        className={`${sizeClass} h-auto drop-shadow-xs`} 
        style={imgStyle}
      />
    </div>
  );
};

/**
 * 4. Dolomiti NordicSki Full Official Logo Component (100% Vector SVG)
 */
export const DolomitiFullLogo: React.FC<{
  variant?: 'original' | 'negative' | 'grayscale' | 'monochrome' | 'skier_track_emblem' | 'horizontal' | 'horizontal_light' | 'badge_card' | 'none';
  className?: string;
  customPrimary?: string;
  customSecondary?: string;
  customAccent?: string;
  isDarkHeader?: boolean;
}> = ({ 
  variant = 'original', 
  className = "h-10",
  isDarkHeader = false
}) => {
  if (variant === 'none') return null;

  if (variant === 'skier_track_emblem') {
    return <DolomitiSkierTrackEmblem className={className} />;
  }

  const isNeg = variant === 'negative' || isDarkHeader || variant === 'horizontal_light';
  const isGrau = variant === 'grayscale';
  const isMono = variant === 'monochrome';

  const dolomitiColor = isMono ? 'currentColor' : isGrau ? BRAND_COLORS.lightGray : BRAND_COLORS.lightTeal;
  const nordicColor = isMono ? 'currentColor' : isNeg ? BRAND_COLORS.white : isGrau ? BRAND_COLORS.darkGray : BRAND_COLORS.darkTeal;
  const trackColor = isMono ? 'currentColor' : isGrau ? BRAND_COLORS.lightGray : BRAND_COLORS.lightTeal;
  const chestColor = isMono ? 'currentColor' : isGrau ? BRAND_COLORS.lightGray : BRAND_COLORS.lightTeal;
  const bodyColor = isMono ? 'currentColor' : isNeg ? BRAND_COLORS.white : isGrau ? BRAND_COLORS.darkGray : BRAND_COLORS.darkTeal;

  let wrapperClass = `inline-flex items-center max-w-full shrink-0 ${className}`;

  if (variant === 'badge_card') {
    wrapperClass += ' px-3 py-1.5 rounded-xl shadow-2xs border bg-white border-[#AAD0D1]';
  }

  return (
    <div className={wrapperClass}>
      <svg 
        viewBox="55 45 835 315" 
        className="h-full w-auto max-h-full object-contain shrink-0"
      >
        <path fill={dolomitiColor} d={PATH_DOLOMITI} />
        <path fill={nordicColor} d={PATH_NORDICSKI} />
        <path fill={trackColor} d={PATH_TRACK_UPPER} />
        <path fill={trackColor} d={PATH_TRACK_LOWER} />
        <path fill={chestColor} d={PATH_SKIER_CHEST} />
        <path fill={bodyColor} d={PATH_SKIER_BODY} />
      </svg>
    </div>
  );
};

export const DolomitiNordicSkiLogo = DolomitiFullLogo;
